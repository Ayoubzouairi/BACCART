...
function loadHistory() {
    ...
    const savedBettingStats = localStorage.getItem('baccaratBettingStats');
    if (savedBettingStats) {
        AppState.bettingStats = JSON.parse(savedBettingStats);
    }
}
...
// تسجيل الرهان ونتيجته
function recordBet(betType, result) {
    AppState.bettingStats.totalBets++;
    AppState.bettingStats.lastBet = betType;
    AppState.bettingStats.lastBetResult = result;
    ...
    AppState.bettingStats.winRate = (AppState.bettingStats.wins / AppState.bettingStats.totalBets) * 100;
    AppState.bettingStats.betHistory.push({
        bet: betType,
        result: result,
...
// تحديث إحصائيات الرهانات
function updateBettingStats() {
    const container = document.getElementById('bettingStats');
    if (!container) return;

    const isArabic = AppState.lang === 'ar-MA';
    
    container.innerHTML = `
        <div class="betting-stats-container">
            <h3>${isArabic ? 'إحصائيات الرهانات' : 'Betting Statistics'}</h3>
            <div class="betting-stats-grid">
                <div class="stat-item">
                    <span class="stat-label">${isArabic ? 'إجمالي الرهانات' : 'Total Bets'}:</span>
                    <span class="stat-value">${AppState.bettingStats.totalBets}</span>
                </div>
                <div class="stat-item">
                    <span class="stat-label">${isArabic ? 'الفوز' : 'Wins'}:</span>
                    <span class="stat-value win-text">${AppState.bettingStats.wins}</span>
                </div>
                <div class="stat-item">
                    <span class="stat-label">${isArabic ? 'الخسارة' : 'Losses'}:</span>
                    <span class="stat-value lose-text">${AppState.bettingStats.losses}</span>
                </div>
                <div class="stat-item">
                    <span class="stat-label">${isArabic ? 'معدل الفوز' : 'Win Rate'}:</span>
                    <span class="stat-value">${AppState.bettingStats.winRate.toFixed(1)}%</span>
                </div>
            </div>
            ${AppState.bettingStats.lastBet ? `
                <div class="last-bet">
                    <p>${isArabic ? 'آخر رهان' : 'Last Bet'}: 
                        <span class="${AppState.bettingStats.lastBet}-text">${isArabic ? 
                            (AppState.bettingStats.lastBet === 'P' ? 'لاعب' : 
                             AppState.bettingStats.lastBet === 'B' ? 'مصرفي' : 'تعادل') : 
                            AppState.bettingStats.lastBet}</span>
                    </p>
                    <p>${isArabic ? 'النتيجة' : 'Result'}: 
                        <span class="${AppState.bettingStats.lastBetResult}-text">${isArabic ? 
                            (AppState.bettingStats.lastBetResult === 'P' ? 'لاعب' : 
                             AppState.bettingStats.lastBetResult === 'B' ? 'مصرفي' : 'تعادل') : 
                            AppState.bettingStats.lastBetResult}</span>
                    </p>
                </div>
            ` : ''}
        </div>
...
async function addResult(result) {
    ...
    // تسجيل الرهان إذا كان هناك توصية
    if (lastRecommendation.recommendation !== 'none') {
        recordBet(lastRecommendation.recommendation, result);
...
async function generateBetRecommendation() {
    ...
    // تعديل الشرط لقبول أي نسبة طالما هي الأعلى
    if (strongestPrediction[1] > Math.max(...Object.values(prediction).filter((_, i) => i !== Object.keys(prediction).indexOf(strongestPrediction[0])))) {
        const recType = strongestPrediction[0];
        const confidence = strongestPrediction[1];
...
function updateUI() {
    ...
    document.querySelector('.model-controls button').textContent = isArabic ? 'تبديل النموذج المتقدم' : 'Toggle Advanced Model';
    document.getElementById('modelStatus').textContent = isArabic ? 
        `النموذج الحالي: ${AppState.useAdvancedModel ? 'المتقدم' : 'الأساسي'}` : 
        `Current model: ${AppState.useAdvancedModel ? 'Advanced' : 'Basic'}`;
...
async function resetData() {
    ...
    if (confirm(confirmMsg)) {
        ...
        document.getElementById('bettingStats').innerHTML = '';
